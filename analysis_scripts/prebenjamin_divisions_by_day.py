...
# Merge to get division_level and category
...
print('=' * 120)
print('ПРЕДЖАМИН - КАКИЕ ДИВИЗИОНЫ В КАКОЙ ДЕНЬ ИГРАЮТ')
print('=' * 120)
print()

# For each day, show all divisions
for day in day_order:
    day_data = prebenjamin[prebenjamin['day_of_week'] == day]
...
print('=' * 120)
print('СВОДНАЯ ТАБЛИЦА - КАЖДЫЙ ДИВИЗИОН ПРЕДЖАМИНА, ВСЕ ДНИ')
print('=' * 120)
print()

# Get all unique divisions for PREBENJAMIN
all_divisions = sorted(prebenjamin['division_level'].unique())

for div in all_divisions:
    div_data = prebenjamin[prebenjamin['division_level'] == div]
    ...
    for day in day_order:
        if day in day_breakdown.index:
            ...
            bar_length = int(pct / 1.5)
            bar = '▓' * bar_length
            print(f'  {day_names_ru[day]:<35} - {count:6d} ({pct:5.1f}%) {bar}')
