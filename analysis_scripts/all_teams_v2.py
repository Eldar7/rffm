#!/usr/bin/env python3
"""
v2 PROOF-OF-CONCEPT: identical to all_teams.py except build_team_lineup_stats(),
build_club_meta(), and build_team_rows() source from output/processed/
rffm_parquet/ via rffm_data.read_table() instead of pd.read_csv(). Imports
build_club_team_cards/list_seasons/norm_id from team_cards_v2, not team_cards,
so the whole chain stays on the Parquet path.
...
def build_team_lineup_stats(season: str) -> dict[str, dict]:
    ...
    for cat in categories:
        lu = data.read_table("match_lineups", season=season, category=cat)[
            ["match_id", "team_id", "player_id", "is_starter"]]
        ...
        cc = data.read_table("match_cards", season=season, category=cat)
        if not cc.empty:
            cc = cc[["team_id", "card_type_label"]]
...
def build_team_rows(season: str) -> list[dict]:
    comps = data.read_table("competitions", season=season)[["competition_id", "category_base"]]
