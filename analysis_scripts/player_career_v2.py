#!/usr/bin/env python3
"""
v2 PROOF-OF-CONCEPT: identical to player_career.py except compute_career_index()
sources from output/processed/rffm_parquet/ via rffm_data.read_table() instead
of pd.read_csv(). Uses read_table("players_by_season", season=...) - NOT the
deduped read_table("players") - because this function's earliest-non-empty-
birth_year-across-seasons logic is season-order-sensitive, defensively (see
rffm_data.py's module docstring: this dataset turned out to have zero players
whose birth_year genuinely changes across seasons, but per-season history is
what would let "earliest" still be computed correctly if that ever isn't
true). Its output still won't match the original byte-for-byte on
birth_year - the original CSVs carry a ".0" float-serialization artifact for
18,594 players that this path's real int typing cleans up; verified that
difference is 100% that formatting artifact, 0% a real value change.
list_fichajugador_seasons()/load_fichajugador_coverage() are unchanged - they
read coverage_manifest.csv, which isn't part of the Parquet ETL (small,
cross-season index, stays CSV).
...
def compute_career_index(seasons: list[str] | None = None) -> dict[str, dict]:
    ...
    for season in seasons:
        part = data.read_table("player_competition_participation", season=season)[["player_id"]]
        players = data.read_table("players_by_season", season=season)[["player_id", "birth_year"]]
        if part.empty or players.empty:
            continue
