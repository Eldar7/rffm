...
- **`output/processed/rffm_parquet/`** — a compact (~20x smaller), lossless
  ...
  **`players.parquet` being gitignored isn't a one-off special case** — every
  table in `output/processed/rffm_parquet/` now follows a general rule for
  *which* of the two copies actually gets committed to git at any given
  time (a table/season still being crawled never gets its Parquet
  committed at all — regenerated on demand instead — until it's closed,
  at which point the CSV becomes redundant and eventually goes away).
  That policy is its own concern, not this file's — see
  **`PARQUET_CLOSURE.md`** for the full rule, how "closed" is decided,
  why some data (`clubs.csv`, `clubs_extended.csv`/`club_teams.csv`) never
  closes, and how it ties into session start-up and site builds.
