...
- **`output/processed/rffm_parquet/`** — a compact (~20x smaller), lossless
  ...
  **Almost every table is one Parquet file *per season*
  ...
  `club_teams.parquet`/`team_club_map.parquet` (genuinely cross-season, no
  season dimension to partition by — `team_club_map` is one row per
  `team_id`, not an append-only log like the other two, but still no
  season dimension since `team_id` is a stable cross-season identity).

  **`players.parquet` is the one exception to "just query the Parquet
  copy" — it's gitignored, not committed** (same git-delta reasoning as
  above, but this table can't be partitioned by season since it's deduped
  to one row per stable `player_id` across all seasons — measured directly:
  committing it anyway cost noticeably more in git history than committing
  the same data as CSV — see `build_parquet.py`'s `PLAYERS_CURRENT_CSV`
  comment). The git-tracked canonical source is
...
- **`team_club_map.csv`** (`team_id` PK, `enrich_team_clubs.py`) — the
  ...
  side), `clubs_representative` (copied from some season's `clubs.csv`), or
  `fichaequipo_direct` (this stage's own live fetch, for whatever's left
  after the first two). Columns: `team_id, club_id, source, source_url,
  scraped_at`. See `OPERATIONS.md`'s dependency order for how this stage
  relates to `clubs`/`club_profiles`.
...
| `club_id` | `codigo_club` | |
| `source` | — | `fichaequipo_direct` / `fichaclub_roster` / `clubs_representative` - see table entry above |
