...
- **`output/processed/rffm_parquet/`** — a compact (~20x smaller), lossless
  ...
  **Almost every table is one Parquet file *per season*
  (`output/processed/rffm_parquet/<table>/<season>.parquet`), not one
  combined file** — glob `*.parquet` across seasons like above, or point at
  one season's file directly. This is deliberate, not just a size
  optimization: see `build_parquet.py`'s module docstring for why (git
  ...
  **`players.parquet` is gitignored, not committed** (same git-delta
  ...
  it's deduped to one row per stable `player_id` across all seasons —
  measured directly: committing it anyway cost noticeably more in git
  history than committing the same data as CSV — see `build_parquet.py`'s
  `PLAYERS_CURRENT_CSV` comment). The git-tracked canonical source is
  `output/processed/rffm/players_current.csv` (deduped to one row per
  `player_id`, cross-season, regenerated by every `build_parquet.py` run).
  If `output/processed/rffm_parquet/players.parquet` doesn't exist on disk
  (e.g. right after a fresh checkout), rebuild just that one file quickly
  with `python analysis_scripts/build_parquet.py --players-only
  --output-dir output/processed/rffm_parquet` before querying it — don't
  run a full rebuild just for this.

  ### The open/closed policy — which Parquet files actually get committed

  `players.parquet` above wasn't a one-off special case — it's the original
  instance of a general rule every table in `output/processed/rffm_parquet/`
  now follows, enforced by `analysis_scripts/parquet_closure.py`:

  - **Closed** — `output/processed/rffm/coverage_manifest.csv` shows
    `status == "complete"` for *every* `category_base` row of that
    `(season, stage)` — strictly `"complete"`, not `complete_with_failures`,
    which `OPERATIONS.md`'s `retry_check.py --fix` can and does reset back
    to `partial` for a later re-crawl attempt, so it isn't a safe "this will
    never change again" signal. `core` is `category_base="ALL"` (one row);
    `acta_partido`/`fichajugador`/`clubs` can have several category rows per
    season and *all* of them must be complete, not just some (so e.g. a
    season with 10 of 11 `fichajugador` categories done still counts as
    open — the simpler of two options considered; the alternative,
    splitting `match_lineups`/etc. into one Parquet file per `(season,
    category)` instead of per season so a category could close
    independently, was deliberately not done, since it'd mean restructuring
    already-committed files — revisit if a single long-tail category
    routinely holds up a whole season's closure for months). Once closed,
    that table/season's Parquet file is committed to git and treated as
    final.
  - **Open** (anything not closed yet, including data that will never
    satisfy strict `"complete"` by design — see below) — the Parquet file
    is **never committed**. It's regenerated on demand instead:
    `build_parquet.py --open-only` (fast — ~2min measured, vs. ~9min for a
    full rebuild — since it only rebuilds the still-open subset, skipping
    every already-closed season/table that's already correct on disk via
    git checkout) runs automatically on session start
    (`.claude/hooks/session-start-open.sh`, async — a query in the first
    couple minutes of a session could in principle race a still-open table,
    accepted since every *closed* season has zero wait regardless) and
    synchronously before every site build (`pages-deploy.yml`). The source
    CSV for an open `(season, stage)` stays the git-tracked copy of record
    — `analysis_scripts/*.py` never reads it directly (see
    `rffm_data.py`/`analysis_scripts/*_v2.py` above), but it exists so
    `build_parquet.py` has something to build the on-demand Parquet from.
  - **`clubs.csv`** never reaches strict `"complete"` in practice — every
    season currently sits at `complete_with_failures` (`OPERATIONS.md`:
    many of its "failures" are permanent structural gaps — phantom teams,
    university teams — not a transient crawl issue worth retrying), and it
    may also keep getting manual corrections indefinitely. It's tiny either
    way (1.6MB CSV / 720KB Parquet total across all 10 seasons as of this
    writing) but is deliberately **not** special-cased to treat
    `complete_with_failures` as "good enough" — it just never closes, the
    same open-forever treatment as everything else that doesn't close, size
    notwithstanding.
  - **`clubs_extended.csv`/`club_teams.csv`** (the `club_profiles` stage)
    have no season dimension and no real "closed" state either — a
    `"complete"` row there only means "the last `enrich_club_profiles.py`
    run finished", not "this data is done": `--force-refetch` is a
    deliberate, repeatable refresh, unlike `retry_check.py`'s genuine
    failure-retry for the other stages. Hardcoded to never close
    (`parquet_closure.STAGES_THAT_NEVER_CLOSE`) regardless of what the
    manifest literally says.
  - **Check current status** any time:
    `python analysis_scripts/parquet_closure.py` (or `--table <name>` /
    `--stage <stage>`) — read-only, safe to run whenever, changes nothing.
  - **Closing** a `(season, stage)` — committing its final Parquet *and*
    deleting its now-redundant source CSV in the same action — is a
    **separate, always manual step**: never run from a scheduled workflow,
    never inferred automatically, only on the project owner's explicit
    request. `parquet-build.yml` itself never does this — it only ever
    commits data that's *already* closed by the rule above.
