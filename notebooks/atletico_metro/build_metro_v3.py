...
# Club Atlético de Madrid S.A.D. (club_id 40024, resolved via team_club_map from
# the team_id=39 deep link this cohort was picked from). Unlike Aravaca's small
# single-team-per-category setup, this club fields many parallel teams per
# category (A/B/C/.../letter) - team_id-based scoping (not club_name_raw
# matching) avoids re-adding a snowball risk here: team_club_map already gives
# the closed set of team_ids for this club_id, no recursive discovery needed.
# The club_name_raw NOT LIKE '%FEM%' filter excludes the separate women's/girls'
# section fielded under the same club_id - a different programme, not this
# cohort's pathway.
CLUB_ID = 40024
_team_ids_df = con.execute(f'''
    SELECT team_id FROM read_parquet('{PARQUET}/team_club_map.parquet') WHERE club_id = {CLUB_ID}
''').df()
...
def get_team_meta_bulk(season):
    """All this club's (non-FEM) teams that season, any category - team_id -> meta."""
    df = con.execute(f'''
    ...
    WHERE t.team_id IN {TEAM_IDS} AND t.club_name_raw NOT LIKE '%FEM%' AND c.phase_label = 'regular_season'
    ORDER BY t.team
    ''').df()
    best = {}
...
for season in SEASONS:
    ...
    WHERE t.team_id IN {TEAM_IDS} AND t.club_name_raw NOT LIKE '%FEM%' AND p.birth_year IN {PIVOT_BIRTH_YEARS}
    ...
    print(season, 'lineup rows:', len(df), 'distinct players:', df['player_id'].nunique(), 'club teams:', len(team_meta[-1]), flush=True)
...
# ---------- build stations (one per real club team_id that season with >=1 qualifying pivot-birth-year player) ----------
stations_out = []
for si, season in enumerate(SEASONS):
...
OUT = 'notebooks/atletico_metro/metro_raw_v3.json'
with open(OUT, 'w', encoding='utf-8') as f:
    json.dump(raw, f, ensure_ascii=False)
with open('notebooks/atletico_metro/intra_transfers_v3.json', 'w', encoding='utf-8') as f:
